...
module Elsewhere
  module Values
    ...
    Money = Struct.new(:amount_minor, :currency, keyword_init: true) do
      def +(other); self.class.new(amount_minor: amount_minor + other.amount_minor, currency: currency); end
      def -(other); self.class.new(amount_minor: amount_minor - other.amount_minor, currency: currency); end
    ...
    Destination = Struct.new(:city_code, :name, :country, :coordinates, keyword_init: true)
    Property = Struct.new(:catalogue_id, :name, :city_code, :coordinates, :rating, :price_level, keyword_init: true)
    Rate = Struct.new(:amount, :basis, :calibration, :handoff_url, keyword_init: true)
    TravelLeg = Struct.new(:origin, :destination, :depart_at, :arrive_at, :carrier, :stops, :duration_min, :amount, :as_of, :booking_url, keyword_init: true)
    GeoFeatures = Struct.new(:distance_to_sea_m, :distance_to_centre_m, :poi_density, :restaurant_count_500m, :nearest_major_road_m, :road_class, :airport_distance_m, keyword_init: true)
