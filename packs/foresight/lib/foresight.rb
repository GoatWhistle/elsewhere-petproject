...
module Foresight
  module Forecasts
    ...
    def for_future(future_id:)
      ...
      { "future_id" => future_id, "generated_at" => Time.now.utc.iso8601, "risks" => [risk], "coverage" => %w[night_noise walkability weather_mismatch transfer_difficulty].map { |type| coverage_entry(type) } }
    end
    # `reason` is only meaningful when a type could not be assessed; the contract types it as a string, so an
    # assessed type omits the key instead of sending null.
    def coverage_entry(type)
      assessed = type == "night_noise"
      entry = { "risk_type" => type, "assessed" => assessed }
      entry["reason"] = "В Phase 0 доступен только geo fixture" unless assessed
      entry
    end
